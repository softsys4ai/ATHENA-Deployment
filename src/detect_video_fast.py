...
def main(_argv):
    ...
    logging.info("yes" + str(tf.config.get_visible_devices()))
    if FLAGS.tiny:
    ...
    try:
        vid = cv2.VideoCapture(int(FLAGS.video), cv2.CAP_GSTREAMER)
    except:
    ...
    #while True: #del this
        ...
        t1 = time.time()
        #print(type(img_in))
        boxes, scores, classes, nums = serve(img_in)
        boxes, scores, classes, nums = serve(img_in)
        boxes, scores, classes, nums = serve(img_in)
        t2 = time.time()
        times.append(t2-t1)
