...
if __name__ == "__main__":
    ...
    for i, image in enumerate(images):
        ...
        frames.append([image, image_w, image_h, images[i]])

    #tell us when we are done loading images
    print("done")
    ...
    for i in range(1):
        ...
        for frame in frames:
            # make prediction
            yhat = model.predict(frame[0])
            boxes = list()
            for i in range(len(yhat)):
                # decode the output of the network
                boxes += decode_netout(yhat[i][0], anchors[i], class_threshold, input_h, input_w)
            # correct the sizes of the bounding boxes for the shape of the image
            correct_yolo_boxes(boxes, frame[2], frame[1], input_h, input_w)
            ...
            #print('\n')
            # draw what we found
            #draw_boxes(frame[3], v_boxes, v_labels, v_scores, 'red')